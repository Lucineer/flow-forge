...
- **Vessel**: Flow Forge
- **Role**: Flow Forge
- **URL**: https://flow-forge.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/flow-forge
- **Branch**: master
...
### Deploy
```bash
cd /tmp/flow-forge && wrangler deploy
```

### Health Check
```bash
curl -s https://flow-forge.casey-digennaro.workers.dev/health
curl -s https://flow-forge.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=flow-forge
...
- **vessel.json capabilities**: workflow-design, vessel-chaining, json-export
...
1. Keep the vessel small. ~226 lines is the current size.
